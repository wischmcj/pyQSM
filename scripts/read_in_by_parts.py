...
def read_file_sections(file,
    ...
    with open(file) as bigfile:
        ...
        # o3d.visualization.draw(pcd)
                    ...
                    # new_cols = arr([tuple((int(((v+1)/2)*255) for v in rgb)) for rgb in pcd_cols])
                    # pcd.colors =  o3d.utility.Vector3dVector(new_cols)
                    # try:
                    #     o3d.visualization.draw(pcd)
                    # except Exception as e:
                    #     print(f'error in draw {e}' )
                    # for idv,vals in enumerate([new_cols[:,0],new_cols[:,1],new_cols[:,2]]):
                    #     oth_desc= stats.describe(np.asarray(vals))
                    #     print(f'{idv}: {oth_desc}')
                    # if debug:
                    #     new_cols = arr([tuple((b,g,r)) for r,g,b in pcd_cols])
                    #     # new_cols = arr([tuple(0 if v<0  else v for v in rgb) for rgb in pcd_cols])
                    #     pcd.colors =  o3d.utility.Vector3dVector(new_cols)
                    #     try:
                    #         o3d.visualization.draw(pcd)
                    #     except Exception as e:
                    ...
                    #     new_cols = arr([tuple((r,b,g)) for r,g,b in pcd_cols])
                    #     # new_cols = arr([tuple(0 if v<0  else v for v in rgb) for rgb in pcd_cols])
                    #     pcd.colors =  o3d.utility.Vector3dVector(new_cols)
                    #     try:
                    #         o3d.visualization.draw(pcd)
                    ...
                    # new_cols = arr([tuple((r,b,g)) for r,g,b in pcd_cols])
                    # breakpoint()
                    # new_cols, hsv = color_distribution(new_cols, space = 'rgb')
                    # pcd = pcd.voxel_down_sample(.05)
                    write_point_cloud(f'data/skeletor/inputs/skeletor_full_{file_num}.pcd',pcd)
                    print(f'Wrote file {file_num} on iter {lineno}')
                    save(f'data/skeletor/inputs/skeletor_full_other_{file_num}.pkl',other_vals)
                    save(f'data/skeletor/inputs/skeletor_full_color_{file_num}.pkl',pcd_cols)
                    lines = []
